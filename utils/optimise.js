...
files.forEach((file) => {
  ...
  console.log(`[${new Date().toLocaleTimeString()}] ${'\x1b[90m'}File ${file} has been copied successfully.${'\x1b[0m'}`);
});

console.log(`[${new Date().toLocaleTimeString()} ${'\x1b[34m'}All assets have been copied successfully.${'\x1b[0m'}`);
...
fs.readdir(postsPath, (err, files) => {
  ...
  files.forEach(file => {
    ...
    if (path.extname(filePath) === '.md') {
      ...
      console.log(`[${new Date().toLocaleTimeString()}] ${'\x1b[90m'}File ${filePath} has been updated successfully.${'\x1b[0m'}`);
    }
  });
});

console.log(`[${new Date().toLocaleTimeString()}] ${'\x1b[34m'}All posts have been updated successfully.${'\x1b[0m'}`);
